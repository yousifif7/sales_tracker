<x-layouts.app title="Inbox | Sales Tracker" heading="Inbox" eyebrow="Email threads">
    <div class="mb-4 grid grid-cols-2 gap-2 md:grid-cols-3 xl:grid-cols-5">
        <a href="{{ route('email-threads.index') }}" @class(['rounded-xl border border-slate-800 bg-slate-950/60 px-3 py-2 transition hover:border-slate-600', 'ring-1 ring-sky-500/40' => ! request()->hasAny(['status', 'unread', 'opened', 'search'])])>
            <p class="text-xs text-slate-400">Total threads</p>
            <p class="mt-0.5 text-lg font-semibold text-white">{{ $stats['total'] }}</p>
        </a>
        <a href="{{ route('email-threads.index', ['unread' => 1]) }}" @class(['rounded-xl border border-slate-800 bg-slate-950/60 px-3 py-2 transition hover:border-slate-600', 'ring-1 ring-sky-500/40' => request('unread') === '1'])>
            <p class="text-xs text-slate-400">Unread replies</p>
            <p class="mt-0.5 text-lg font-semibold text-sky-300">{{ $stats['unread'] }}</p>
        </a>
        <a href="{{ route('email-threads.index', ['status' => 'awaiting_reply']) }}" @class(['rounded-xl border border-slate-800 bg-slate-950/60 px-3 py-2 transition hover:border-slate-600', 'ring-1 ring-sky-500/40' => request('status') === 'awaiting_reply'])>
            <p class="text-xs text-slate-400">Awaiting reply</p>
            <p class="mt-0.5 text-lg font-semibold text-white">{{ $stats['awaiting_reply'] }}</p>
        </a>
        <a href="{{ route('email-threads.index', ['status' => 'responded']) }}" @class(['rounded-xl border border-slate-800 bg-slate-950/60 px-3 py-2 transition hover:border-slate-600', 'ring-1 ring-sky-500/40' => request('status') === 'responded'])>
            <p class="text-xs text-slate-400">Responded</p>
            <p class="mt-0.5 text-lg font-semibold text-amber-200">{{ $stats['responded'] }}</p>
        </a>
        <a href="{{ route('email-threads.index', ['opened' => 1]) }}" @class(['rounded-xl border border-slate-800 bg-slate-950/60 px-3 py-2 transition hover:border-slate-600', 'ring-1 ring-sky-500/40' => request('opened') === '1'])>
    ...
    <section class="panel mb-6">
        <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
            <p class="text-sm text-slate-400">Replies attach to your sent threads only. Deleted threads stay in trash.</p>
        ...
        <form method="get" class="grid gap-3 lg:grid-cols-7">
            <input class="input mt-0 lg:col-span-2" name="search" value="{{ request('search') }}" placeholder="Search contact, email, subject">
    ...
    <x-data-table>
        ...
        <tbody class="divide-y divide-slate-800" id="inbox-rows">
            @forelse ($threads as $thread)
                ...
                @empty
                    <tr>
                        <td colspan="7" class="text-center text-slate-500">No email threads yet. Send an outreach email to start one.</td>
                    </tr>
                @endforelse
            </tbody>
        </x-data-table>
